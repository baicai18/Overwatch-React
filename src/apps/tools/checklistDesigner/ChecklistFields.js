...
class ChecklistFields extends React.Component { 
    constructor(props){
        super(props);
        this.lastKey=0;
    ...
    render() {
        return (
            <Card>
                ...
                <Card.Body>
                <DraggableList 
                    key={this.state.formData.fields} 
                    keyName={"NULL"}
                    list={this.state.formData?this.state.formData.fields:[]} 
                    onListChanged={this.onListChanged}
                    mapFunction={(x,index)=>{
                        return <ChecklistField key={index.toString() + x.key} index={index} field={x} onDataChanged={this.onFieldDataChanged} onDeleteField={this.onDeleteField} roles={this.props.roles}/>
                        }
                    }/>
                </Card.Body>
                {/* <Card.Body>
