...
import {Card, Button, Form, Row, Col, Nav, Table} from 'react-bootstrap';
...
class FileSubmit extends React.Component{
    ...
    componentDidUpdate(props){
        ...
        if(props.project !== this.props.project && JSON.strinigfy(this.props.project) !== '{}'){
            this.setState({
                project:this.props.project
            })

        }
        if(props.revision !== this.props.revision && JSON.strinigfy(this.props.revision) !== '{}'){
            this.setState({
                revision:this.props.revision
            })

        }
        if(props.requirement !== this.props.requirement && JSON.strinigfy(this.props.requirement) !== '{}'){
            this.setState({
            ...
            <div>
                ...
                <Card>
                    ...
                    <Card.Body style={{overflow:"auto"}}>
                            ...
                            />
                            {/* {
                                this.state.deliverable&&this.state.deliverable.files?
                                <Table>
                                    <thead>
                                        <tr><th>Filename</th><th>Create Date</th><th>Created By</th><th>Actions</th></tr>
                                    </thead>
                                    <tbody>
                                        {
                                        this.state.deliverable.files.map((x, index)=>{
                                            return <tr>
                                                ...
                                                <td>
                                                    {moment(x.create_date).format('YYYY-MM-DD hh:mma')}
                                                </td>
                                                <td>
                                                    {x.created_by}
                                                </td>
                                            ...
                                            return <Row>
                                                        ...
                                                        <Col className="col-auto">
                                                            <Nav.Link pill variant="primary" onClick={()=>{this.getFile(x.file)}}>{moment(x.create_date).format('YYYY-MM-DD hh:mma')}</Nav.Link>
                                                        </Col>
                                                        <Col className="col-auto">
                                                            <Button variant="danger" size="sm" onClick={()=>{this.deleteFile(x.file, index)}}>X</Button>
                                                        </Col>
    ...
    deleteFile = async (file, index)=>{
        try{
            let data = await SDK.RFQ.deleteDeliverableFile(this.state.rfq_number, this.state.requirement._id, this.state.deliverable._id, file);
    ...
    uploadFile = ()=>{
        if(this.state.loadedFile){
            SDK.RFQ.submitDeliverableFile(this.state.rfq_number, this.state.requirement._id, this.state.deliverable._id,this.state.loadedFile)
            .then(data=>{
                if(data.files){
                    //inserted
        ...
        try{
            if(this.state.selectedFiles){
                for(let x = 0; x < this.state.selectedFiles.length; x++){
                    let data = await SDK.RFQ.submitDeliverableFile(this.state.rfq_number, this.state.requirement._id, this.state.deliverable._id,this.state.selectedFiles[x])
                    console.log("SAVED");
                    console.log(data)
                    if(data.files){
                        //inserted
