...
class BomSubmit extends React.Component{
    ...
    componentDidUpdate(props){
        ...
        if(props.project !== this.props.project && JSON.stringify(this.props.project) !== `{}`){
            this.setState({
                project:this.props.project
            })

        }
        if(props.revision !== this.props.revision && JSON.stringify(this.props.revision) !== `{}`){
            this.setState({
                revision:this.props.revision
            })

        }
        if(props.requirement !== this.props.requirement && JSON.stringify(this.props.requirement) !== `{}`){
            this.setState({
    ...
    uploadBOM = ()=>{
        if(this.state.parsedBOM){
            SDK.RFQ.submitDeliverableBOM(this.state.rfq_number, this.state.requirement._id, this.state.deliverable._id,this.state.parsedBOM)
            .then(data=>{
    ...
    uploadFile = ()=>{
        if(this.state.excelFile){
            SDK.RFQ.submitDeliverableFile(this.state.rfq_number, this.state.requirement._id, this.state.deliverable._id,this.state.selectedFiles[0])
            .then(data=>{
                if(data){
                    let progressInfos = this.state.progressInfos;
                    progressInfos.push(this.state.selectedFiles[0].name)
    ...
    deleteFile = (file, index)=>{
        SDK.RFQ.deleteDeliverableFile(this.state.rfq_number, this.state.requirement._id, this.state.deliverable._id, file)
        .then(data=>{
            if(data){
